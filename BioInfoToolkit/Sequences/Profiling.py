...
class SequencesProfile:
    ...
    def __init__(self, sequences: list[str], alphabet: list[str] | None, pseudocounts: int = 0) -> None:
        if not alphabet:
            alphabet = sorted(list(set(symbol for seq in sequences for symbol in seq)))
        ...
        profile: list[dict[str, int]] = []
        for nucs in zip(*sequences):
            counts = dict(Counter(nucs))
    ...
    def to_list_matrix(self) -> list[list[float]]:
        profile_mat: list[list[float]] = [[d[key] for d in self.profile] for key in self.alphabet]
        return profile_mat
...
def findProfileMostProbableKmers(text: str, k: int, profile_mat: list[list[float]], alphabet: list[str]):
    ...
    Args:
        text (str): a string
        k (int): k-mer length
        profile_mat (list[list[float]]): an n x k matrix
