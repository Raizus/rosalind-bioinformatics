...
from BioInfoToolkit.RuleBasedModel.utils.action_parsers import GenerateNetworkDict, SimulationParams
...
def substitute_rate_expressions(
    reaction_rules: OrderedDict[int, ReactionRule],
    parameters_block: ParametersBlock
):
    """substitute reaction rate expressions by new variables
    if they're not variables already, and add the new variables 
    to the parameters block

    Args:
        reaction_rules (OrderedDict[int, ReactionRule]): _description_
        parameters_block (ParametersBlock): _description_
    """
    r_law_id = 1
    for _, reaction in reaction_rules.items():
        expr = reaction.forward_rate
        # expression is already a declared variable
        if expr in parameters_block.items:
            continue

        # iterate until a valid varaible name is found
        while True:
            var_name = f"_rateLaw{r_law_id}"
            if var_name not in parameters_block.items:
                break
            r_law_id += 1

        # create and add new parameter to param block
        new_param = Parameter(var_name, expr)
        parameters_block.add_parameter(new_param)
...
class ReactionNetwork:
    ...
    def generate_reactions(self, model: Model,
        ...
        """Generates the reaction network from the model rules and seed species. The network
        is generated by iteratively applying the rules to a list of species 
        (starting from the seed species), and augmenting the species list with the new species
        created when applying each reaction rule to the current list of species.

        Args:
            model (Model): _description_
            max_iter (int | None, optional): Maximum number of iterations. Defaults to None.
                If None, then it will keep iterating until no more new species are generated.
                This can result in infinite loops when polimerazation occurs, if max_stoich
                is not provided.
            max_stoich (dict[str, int] | None, optional): Maximum stoichiometry for any 
                given species. Limits the maximum number of molecules in any given complex.
                Defaults to None.
        """
        ...
        # substitute reaction rate expressions by new variables,
        # if they're not variables already
        substitute_rate_expressions(reaction_rules, self.parameters_block)
    ...
    def as_string(self) -> str:
        """Returns the string that represents the networks. 
        This is the string that makes up the .net file
    ...
    def get_rate_constants(self) -> OrderedDict[int, float]:
        """Generates an ordered dictionary mapping reaction id's to the respective 
        reaction rates

        Raises:
            TypeError: Error raised if the result of evaluating the rate expression is
                not an integer or float

        Returns:
            OrderedDict[int, float]: reaction rate dictionary
        """
    ...
    def gillespie_simulation(self, params: SimulationParams):
